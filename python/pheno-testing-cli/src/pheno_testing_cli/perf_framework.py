...
class LoadTester:
    ...
    def run_load_test(
        ...
        print(
            f"🔄 Running load test: {iterations} iterations, {concurrent_users} concurrent users",
        )
            ...
            metrics.extend(
                [
                    ...
                    PerformanceMetric(
                        ...
                        value=statistics.quantiles(valid_results, n=20)[18]
    ...
    def _evaluate_performance(self, metrics: list[PerformanceMetric]) -> str:
        ...
        avg_response_time = next(
            (m.value for m in metrics if m.name == "avg_response_time"),
            0,
        )
    ...
    def _generate_recommendations(self, metrics: list[PerformanceMetric]) -> list[str]:
        ...
        throughput = next((m.value for m in metrics if m.name == "throughput"), 0)
        memory_usage = next(
            (m.value for m in metrics if m.name == "memory_usage_mb"),
            0,
        )

        if avg_response_time > 2.0:
            recommendations.append(
                "Consider optimizing response time - current average is high",
            )

        if throughput < 50:
            recommendations.append(
                "Throughput is low - consider performance optimization",
            )
...
class StressTester:
        ...
        if valid_results:
            metrics.extend(
                [
                    ...
                    PerformanceMetric(
                        name="error_rate",
                        value=((len(results) - len(valid_results)) / len(results))
                        * 100,
                        unit="percent",
                        timestamp=time.time(),
                    ),
    ...
    def _generate_stress_recommendations(
        ...
        if error_rate > 5:
            recommendations.append(
                f"High error rate ({error_rate:.1f}%) - investigate stability issues",
            )

        if avg_response_time > 5:
            recommendations.append(
                f"High response time ({avg_response_time:.2f}s) - optimize performance",
            )

        if peak_memory > 1000:
            recommendations.append(
                f"High memory usage ({peak_memory:.1f}MB) - consider memory optimization",
            )
...
class PerformanceTestingFramework:
    ...
    def _generate_overall_recommendations(self) -> list[str]:
        ...
        avg_response_times = [
            m.value for m in all_metrics if m.name == "avg_response_time"
        ]
        if avg_response_times and statistics.mean(avg_response_times) > 2.0:
            recommendations.append(
                "Overall response times are high - consider performance optimization",
            )
        ...
        if error_rates and any(rate > 5 for rate in error_rates):
            recommendations.append(
                "High error rates detected - investigate stability issues",
            )
