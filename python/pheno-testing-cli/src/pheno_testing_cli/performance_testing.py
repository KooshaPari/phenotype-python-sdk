        ...
        if response_times:
            ...
            metrics.append(
                PerformanceMetric(
                    ...
                    percentile_50=float(np.percentile(response_times, 50)),
                    percentile_95=float(np.percentile(response_times, 95)),
                    percentile_99=float(np.percentile(response_times, 99)),
    ...
    def _generate_recommendations(
        ...
        if error_rate > config.error_threshold:
            recommendations.append(
                f"High error rate ({error_rate:.1f}%) - investigate error causes",
            )

        if latency > config.max_response_time:
            recommendations.append(
                f"High latency ({latency:.1f}ms) - optimize response time",
            )

        if throughput < config.target_throughput * 0.8:
            recommendations.append(
                f"Low throughput ({throughput:.1f} req/s) - optimize performance",
            )
...
class StressTester:
    ...
    def _generate_stress_recommendations(
        ...
        if error_rate > 50:
            recommendations.append(
                "System failed under stress - implement better error handling",
            )

        if latency > 5000:
            recommendations.append(
                "System too slow under stress - optimize performance",
            )
...
class PerformanceTestingFramework:
    ...
    def _run_cpu_intensive_test(self) -> PerformanceTestResult:
        ...
        if duration > 10:  # More than 10 seconds
            status = "fail"
            recommendations = [
                "CPU performance below expectations",
    ...
    def _generate_performance_report(self) -> dict[str, Any]:
        ...
        avg_throughput = statistics.mean(
            [r.throughput for r in self.test_results if r.throughput > 0],
        )
        avg_latency = statistics.mean(
            [r.latency for r in self.test_results if r.latency > 0],
        )
...
"""
        ...
        for result in report["test_results"]:
            status_emoji = (
                "✅"
                if result["status"] == "pass"  # type: ignore
                else "❌"
                if result["status"] == "fail"  # type: ignore
                else "⚠️"
            )
...
def main():
    ...
    if args.load_test:
        ...
        config = next(
            (c for c in framework.load_test_configs if c.name == args.load_test),
            None,
        )
        if config:
        ...
        if isinstance(test_results, list):
            for result in test_results:
                if isinstance(result, dict):
                    ...
                    output += f"  {status_emoji} {result.get('test_name', 'Unknown')}: {result.get('status', 'Unknown')}\n"
    else:
