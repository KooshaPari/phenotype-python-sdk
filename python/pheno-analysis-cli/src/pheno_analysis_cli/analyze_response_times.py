#!/usr/bin/env python3
"""Response Time Analysis Script for Pheno-SDK.
...
class ResponseTimeAnalyzer:
    ...
    def __init__(self, base_url: str = "https://api.pheno.com"):
    ...
    def calculate_statistics(self) -> dict[str, Any]:
        ...
        successful_times = [
            rt["response_time_ms"] for rt in self.response_times if rt["success"]
        ]
    ...
    def generate_report(self) -> str:
        ...
        for percentile in self.percentiles:
            report.append(
                f"  P{percentile}: {stats[f'p{percentile}_response_time']:.2f}ms",
            )
        ...
        if slow_endpoints:
            ...
            for endpoint in slow_endpoints[:5]:  # Show top 5
                report.append(
                    f"  {endpoint['endpoint']}: {endpoint['response_time_ms']:.2f}ms",
                )
...
def main():
    ...
    if not args.endpoints:
        args.endpoints = ["/health", "/sdk/v1/status", "/auth/verify", "/metrics"]
    ...
    if args.json:
        print(
            json.dumps(
                {"statistics": stats, "response_times": analyzer.response_times},
                indent=2,
            ),
        )
    else:
        ...
        if "error" in stats:
            ...
            print(
                f"  Average Response Time: {stats.get('average_response_time', 0):.2f}ms",
            )
