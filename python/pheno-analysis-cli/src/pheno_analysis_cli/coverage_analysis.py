#!/usr/bin/env python3
"""Coverage Analysis Script for Pheno-SDK.
...
def generate_coverage_report() -> str:
    ...
    report = []
    report.append("Pheno-SDK Coverage Analysis Report")
    report.append("=" * 40)
    ...
    if analysis["coverage_data"]:
        total_coverage = (
            analysis["coverage_data"].get("totals", {}).get("percent_covered", 0)
        )
        ...
        low_coverage_files = [
            ...
            if data.get("summary", {}).get("percent_covered", 0) < 75
        ]

        if low_coverage_files:
            report.append("\nFiles with Low Coverage (<75%):")
