...
class ReleaseAutomation:
    ...
    def create_release(  # noqa: PLR0911
        self,
        release_type: str,
        ...
        new_version = self._calculate_new_version(
            current_version, release_type, prerelease, custom_version,
        )
        if not new_version:
            return self._generate_release_report()
    ...
    def _complete_step(self, success: bool, output: str = "", error: str = "") -> None:
        ...
        if self.release_steps:
            ...
            current_step.duration = (
                time.time() - current_step.duration
            )  # This is a bit hacky
    ...
    def _validate_environment(self) -> bool:
        ...
        try:
            ...
            if not version_file.exists():
                self._complete_step(
                    False, "", f"Version file not found: {self.config['version_file']}",
                )
                return False
    ...
    def _get_current_version(self) -> str | None:
        ...
        try:
            ...
            self._complete_step(
                False, "", f"Unsupported version file format: {version_file.suffix}",
            )
            return None
        ...
        try:
            if custom_version:
                ...
                if release_type == "major":
                    ...
                    self._complete_step(
                        False, "", f"Invalid release type: {release_type}",
                    )
                    return None
...
"""
    ...
    def _generate_release_notes(self, new_version: str, changelog: str) -> str  # noqa: PLR0912:
...
"""
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "commit",
                    "-m",
                    f"Release {self.current_release.version if self.current_release else 'unknown'}",
                ],
                check=False,
                capture_output=True,
                text=True,
            ...
            if result.returncode != 0:
                self._complete_step(
                    False, "", f"Failed to commit changes: {result.stderr}",
                )
                return False
            ...
            if result.returncode != 0:
                self._complete_step(
                    False, "", f"Failed to push changes: {result.stderr}",
                )
                return False
    ...
    def _generate_release_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "release_info": asdict(self.current_release)
