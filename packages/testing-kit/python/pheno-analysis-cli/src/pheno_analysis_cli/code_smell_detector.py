...
class CodeSmellDetector:
    ...
    def analyze_file(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            file_smells.extend(
                self._detect_duplicate_code_blocks(tree, file_path, content),
            )
    ...
    def _detect_duplicate_code(
        ...
        for line_numbers in line_groups.values():
    ...
    def _detect_feature_envy(self, tree: ast.AST, file_path: Path) -> list[CodeSmell]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if (
                    external_calls > internal_calls * 2
                ):  # More external than internal calls
                    smells.append(
                        CodeSmell(
                            type="feature_envy",
                            severity="medium",
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if len(params) > 3:
                    ...
                    if (
                        len(set(param_names)) < len(param_names) * 0.8
                    ):  # Many similar names
                        smells.append(
                            CodeSmell(
                                type="data_clump",
    ...
    def _detect_parallel_inheritance(
        ...
        for i, class1 in enumerate(classes):
            for class2 in classes[i + 1 :]:
                ...
                if (
                    len(common_methods) > len(methods1) * 0.5
                ):  # More than 50% common methods
                    smells.append(
                        CodeSmell(
