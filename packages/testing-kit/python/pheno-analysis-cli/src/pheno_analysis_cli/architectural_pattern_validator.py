...
class ArchitecturalPatternValidator:
    ...
    def analyze_file(self, file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            file_violations.extend(
                self._validate_hexagonal_architecture(tree, file_path),
            )
            ...
            file_violations.extend(
                self._validate_interface_segregation(tree, file_path),
            )
            file_violations.extend(
                self._validate_single_responsibility(tree, file_path),
            )
            file_violations.extend(
                self._validate_open_closed_principle(tree, file_path),
            )
            ...
            file_violations.extend(
                self._validate_microservices_patterns(tree, file_path),
            )
            file_violations.extend(self._validate_api_gateway_pattern(tree, file_path))
            file_violations.extend(
                self._validate_circuit_breaker_pattern(tree, file_path),
            )
            ...
            file_violations.extend(
                self._validate_event_driven_architecture(tree, file_path),
            )
            file_violations.extend(self._validate_cqrs_pattern(tree, file_path))
    ...
    def _validate_hexagonal_architecture(
        ...
        for class_node in classes:
            ...
            if any(
                keyword in class_name for keyword in self.patterns["hexagonal"]["ports"]
            ):
                ...
                for keyword in self.patterns["hexagonal"]["adapters"]
            ):
    ...
    def _validate_clean_architecture(
        ...
        for class_node in classes:
            ...
            if any(
                keyword in class_name
                for keyword in self.patterns["clean_architecture"]["entities"]
            ):
            ...
            if isinstance(node, ast.ClassDef):
                ...
                for method in node.body:
                    if isinstance(method, ast.FunctionDef):
                        if_elif_count = len(
                            [n for n in ast.walk(method) if isinstance(n, ast.If)],
                        )
                ...
                if node.bases:  # Has base classes
                    ...
                    for method in node.body:
                        if isinstance(method, ast.FunctionDef):
                            ...
                            raises = [
                                n for n in ast.walk(method) if isinstance(n, ast.Raise)
                            ]
                            if raises:
                                violations.append(
                                    ArchitecturalViolation(
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if init_method:
                    ...
                    if len(params) > 1:  # More than just self
                        ...
                        if (
                            len(assignments) < len(params) - 1
                        ):  # Not all params assigned
                            violations.append(
                                ArchitecturalViolation(
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if "service" in class_name:
                    ...
                    error_handling = [
                        m
                        for m in node.body
                        if isinstance(m, ast.FunctionDef) and "try" in str(m)
                    ]
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if "gateway" in class_name or "proxy" in class_name:
                    ...
                    routing_methods = [
                        ...
                        if isinstance(m, ast.FunctionDef)
                        and any(
                            route in m.name.lower()
                            for route in ["route", "forward", "proxy"]
                        )
                    ]
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if "bulkhead" in class_name:
                    ...
                    isolation_methods = [
                        ...
                        and any(
                            iso in m.name.lower()
                            for iso in ["isolate", "separate", "partition"]
                        )
                    ]
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if "saga" in class_name:
                    ...
                    compensation_methods = [
                        ...
                        if isinstance(m, ast.FunctionDef)
                        and any(
                            comp in m.name.lower()
                            for comp in ["compensate", "rollback", "undo"]
                        )
                    ]
