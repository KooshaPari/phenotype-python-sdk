...
class PatternAnalysisResults:
    ...
    def add_finding(self, finding: PatternFinding) -> None:
        ...
        self.category_counts[finding.category] = (
            self.category_counts.get(finding.category, 0) + 1
        )
        self.severity_counts[finding.severity] = (
            self.severity_counts.get(finding.severity, 0) + 1
        )
...
class VibePatternVisitor(BasePatternVisitor):
    ...
    def visit_Constant(self, node: ast.Constant) -> None:
        ...
        if isinstance(node.value, str) and len(node.value) > 10:
            ...
            # Skip if this is a docstring (first statement in module/class/function)
            if node.value.isupper() and "_" in node.value:
                self.add_finding(
                    ...
                    "vibe-pattern",
                    "low",
                    f"Magic string detected: '{node.value[:20]}...'",
...
class ArchitecturalPatternVisitor(BasePatternVisitor):
    ...
    def _determine_layer(self, file_path: Path) -> str:
        ...
        if "domain" in path_str or "entities" in path_str:
            return "domain"
        if (
            "application" in path_str
            or "services" in path_str
            or "use_cases" in path_str
        ):
            return "application"
    ...
    def _determine_imported_layer(self, module_name: str) -> str:
        ...
        if any(
            ...
            for keyword in ["domain", "entities", "models"]
        ):
            return "domain"
        if any(
            keyword in module_name.lower()
            for keyword in ["application", "services", "use_cases"]
        ):
            return "application"
        if any(
            keyword in module_name.lower()
    ...
    def _check_single_responsibility(self, node: ast.ClassDef) -> None:
        ...
        for method in methods:
            ...
            if any(
                ...
                keyword in method_name for keyword in ["validate", "check", "verify"]
            ):
                responsibilities["validation"].append(method)
            elif any(
                keyword in method_name
                for keyword in ["format", "render", "display", "print"]
            ):
    ...
    if args.json:
        ...
        if args.category:
            output["findings"] = [
                ...
                if isinstance(f, dict) and f.get("category") == args.category
            ]
        ...
        if args.severity:
            output["findings"] = [
                f
                for f in output["findings"]
                if isinstance(f, dict) and f.get("severity") == args.severity
            ]
