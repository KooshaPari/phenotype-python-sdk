...
class TestAutomationSuite:
    ...
    def _run_tests_sequential(
        ...
        for suite in test_suites:
            ...
            print(
                f"{status_emoji} {suite.name}: {result.status} ({result.duration:.1f}s)",
            )
    ...
    def _generate_comprehensive_report(
        ...
        score = 100 if total_tests == 0 else passed_tests / total_tests * 100
        ...
        for result in coverage_results:
            if result.metrics and result.metrics.get("coverage_percent", 100) < 80:
                recommendations.append(
                    f"Improve test coverage (currently {result.metrics.get('coverage_percent', 0)}%)",
                )

        # Check for performance issues
        performance_results = [
            r for r in results if "performance" in r.suite_name.lower()
        ]
...
def main():
    ...
    try:
        ...
        print(
            f"  Status: {'✅ PASS' if summary.get('overall_status') == 'pass' else '❌ FAIL'}",
        )
