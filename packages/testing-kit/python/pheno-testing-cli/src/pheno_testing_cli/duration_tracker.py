#!/usr/bin/env python3
"""Test Duration Tracking Script for Pheno-SDK.
...
def run_test_duration_analysis() -> dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(
            ["pytest", "--durations=0", "--durations-min=0.1", "-v"],
            check=False,
            capture_output=True,
            text=True,
        )
            ...
            if "passed" in line and "[" in line and "]" in line:
                ...
                if len(parts) >= 2:
                    ...
                    try:
                        ...
                        if duration > 3.0:  # Consider tests > 3s as slow for SDK
        ...
        return {
            ...
            "average_duration": (
                sum(d["duration"] for d in durations) / len(durations)
                if durations
                else 0
            ),
...
def identify_slow_tests(threshold: float = 3.0) -> list[dict[str, Any]]:
    """
    ...
    return [
        test
        for test in analysis.get("all_durations", [])
...
def generate_duration_report() -> str:
    ...
    report = []
    report.append("Pheno-SDK Test Duration Report")
    report.append("=" * 35)
    ...
    report.append(f"Slow Tests (>3s): {analysis['slow_test_count']}")
