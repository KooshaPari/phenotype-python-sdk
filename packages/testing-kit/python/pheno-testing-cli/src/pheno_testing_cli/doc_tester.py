...
class DocumentationTester:
            ...
            if "import" in code and "from" in code:
                ...
                if len(import_lines) > 1:
                    ...
                    for i in range(len(import_lines) - 1):
                        if import_lines[i].startswith("from") and import_lines[
                            i + 1
                        ].startswith("import"):
                            self.results.append(
                                DocTestResult(
    ...
    def _test_api_documentation(self) -> None:
        ...
        api_docs = (
            list(self.docs_path.rglob("*api*.md")) if self.docs_path.exists() else []
        )
