...
def cache_stats() -> dict[str, float]:
    ...
    now = time.time()
    return {
        "entries": float(len(_client_cache)),
        "ttl_seconds": float(_CACHE_TTL),
        "oldest_age_seconds": float(
            max((now - created for _, created in _client_cache.values()), default=0.0),
        ),
    }


__all__ = ["MissingSupabaseConfig", "cache_stats", "get_supabase", "reset_client_cache"]
