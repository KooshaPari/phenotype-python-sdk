...
class AsyncConnectionPool:
    ...
    def __init__(self, pool_name: str, config: ConnectionPoolConfig | None = None):
        """Prime the async connection pool with configuration defaults.

        Args:
            pool_name: Human friendly identifier used for metrics/logging.
            config: Optional pool configuration overrides.

        Raises:
            ImportError: If aiohttp is not installed in the runtime.
        """
        if not AIOHTTP_AVAILABLE:
            raise ImportError(
                "aiohttp is required for AsyncConnectionPool. Install with: pip install aiohttp",
            )
...
class SyncConnectionPool:
    ...
    def __init__(self, pool_name: str, config: ConnectionPoolConfig | None = None):
        if not HTTPX_AVAILABLE:
            raise ImportError(
                "httpx is required for SyncConnectionPool. Install with: pip install httpx",
            )
