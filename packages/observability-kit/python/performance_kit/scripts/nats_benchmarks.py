...
class NATSBenchmarkRunner:
    ...
    async def run_communicator_benchmarks(self) -> list[BenchmarkResult]:
        ...
        try:
            ...
            from src.shared.utilities.nats_communicator import (
                NATSCommunicator,
                NATSConfig,
            )
        ...
        try:
            with patch("utils.nats_communicator.nats") as mock_nats:
                ...
                if latencies:
                    ...
                    result.add_metric(
                        "p95_latency", sorted(latencies)[int(len(latencies) * 0.95)], "ms",
                    )
                    result.add_metric(
                        "p99_latency", sorted(latencies)[int(len(latencies) * 0.99)], "ms",
                    )
    ...
    async def run_streaming_benchmarks(self) -> list[BenchmarkResult]:
        ...
        result = BenchmarkResult(
            "jetstream_throughput", "JetStream persistent messaging throughput",
        )
        result.start()
        ...
        try:
            ...
            from src.shared.utilities.nats_streaming import NATSStreamingManager
            ...
            with patch("utils.nats_streaming.nats"):
                ...
                # Mock JetStream
                mock_js = AsyncMock()
                mock_js.publish.return_value = MagicMock(
                    stream="TEST_STREAM", seq=1, duplicate=False,
                )
                ...
                for i in range(message_count):
                    success = await manager.publish_persistent(
                        f"test.stream.{i}", {"data": f"message_{i}", "timestamp": time.time()},
                    )
                    if success:
    ...
    async def run_discovery_benchmarks(self) -> list[BenchmarkResult]:
        ...
        try:
            ...
            with patch("utils.nats_communicator.nats") as mock_nats:
                ...
                result.add_metric(
                    "registration_rate", service_count / registration_time, "services/sec",
                )
    ...
    async def run_monitoring_benchmarks(self) -> list[BenchmarkResult]:
        ...
        result = BenchmarkResult(
            "metrics_recording", "Metrics recording and aggregation performance",
        )
        result.start()
        ...
        try:
            ...
            from src.shared.utilities.nats_monitoring import NATSMonitoringSystem
    ...
    async def run_integration_benchmarks(self) -> list[BenchmarkResult]:
        ...
        result = BenchmarkResult(
            "end_to_end_integration", "Complete system integration performance",
        )
        result.start()
        ...
        try:
            ...
            from src.shared.agents import AgentDiscoveryService, AgentService
            from src.shared.utilities.nats_communicator import (
                NATSCommunicator,
                NATSConfig,
            )
            ...
            with patch("utils.nats_communicator.nats") as mock_nats:
                ...
                monitor = NATSMonitoringSystem(
                    communicator, streaming, discovery, redis_client=mock_redis,
                )
                await monitor.start()
                ...
                for i in range(200):
                    await monitor.record_metric(
                        f"integration_metric_{i % 20}", float(i), {"test": "integration"},
                    )
                    metrics_recorded += 1
                ...
                for _i in range(50):
                    from src.shared.agents import ServiceQuery
                ...
                result.add_metric(
                    "overall_throughput",
                    (services_registered + messages_published + metrics_recorded) / scenario_time,
                    "operations/sec",
                )
    ...
    def print_results(self, results: list[BenchmarkResult]) -> None:
        ...
        """
        print("\n" + "=" * 80)
        logger.info("NATS PERFORMANCE BENCHMARK RESULTS")
        print("=" * 80)
...
async def main():
    ...
    parser.add_argument(
        "--component",
        choices=["communicator", "streaming", "discovery", "monitoring", "integration", "all"],
        default="all",
